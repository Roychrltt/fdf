...
void	draw_instructions(t_vars *vars)
{
	...
	mlx_string_put(mlx, win, 15, y += 20, 0x3A3A3A, "Move: W, A, S, D");
	mlx_string_put(mlx, win, 15, y += 20, 0x3A3A3A, "Zoom: - +");
	mlx_string_put(mlx, win, 15, y += 20, 0x3A3A3A, "Flatten: arrow up / down");
	mlx_string_put(mlx, win, 15, y += 20, 0x3A3A3A, "Rotate: left / right");
